...
void annim_right(framing *lvl)
{
    ...
    if (lvl->map_txt.x == 0)
        player_right(lvl);
    if (lvl->map_txt.x < 0 && lvl->player_center_h == 1)
        map_right(lvl);
}

int go_right(framing *lvl)
{
    int x = lvl->player_x;
    int y = lvl->player_y;

    lvl->player_txt.status = 2;
    if (lvl->map[y][x + 1] != '\0' && lvl->save_map[y][x + 1] == ' ') {
