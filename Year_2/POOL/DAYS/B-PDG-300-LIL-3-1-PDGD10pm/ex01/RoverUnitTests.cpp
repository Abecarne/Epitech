...
void testWaterReclaimer()
{
    ...
    try {
        ...
        assert(std::strcmp(error.what(), "Cannot use water if the reclaimer hasn't started.") == 0);
        assert(error.getComponent() == "WaterReclaimer");
        hasExcept = true;
    }
    assert(hasExcept);
...
void testEngine()
{
    ...
    try {
        ...
        hasExcept = true;
        assert(error.getComponent() == "Engine");
        assert(std::strcmp(error.what(), "Cannot reach destination (7, 8).") == 0);
    }
    assert(hasExcept);
    hasExcept = false;
    try {
        ...
        hasExcept = true;
        assert(error.getComponent() == "Engine");
        assert(std::strcmp(error.what(), "Cannot reach destination (9, 8).") == 0);
    }
    assert(hasExcept);
    hasExcept = false;
    try {
        ...
        hasExcept = true;
        assert(error.getComponent() == "Engine");
        assert(std::strcmp(error.what(), "Cannot reach destination (-10, 7).") == 0);
    }
    assert(hasExcept);
}
